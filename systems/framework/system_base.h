...
inline std::string GetSystemType(const systems::SystemBase&);

template <typename T>
struct nice_type_name<T, std::enable_if_t<
              std::is_base_of_v<systems::SystemBase, T>>> {
  static std::string Get(const systems::SystemBase& thing) {
    return GetSystemType(thing);
  }
};
}  // namespace internal
...
class SystemBase : public internal::SystemMessageInterface {
 ...
 protected:
  ...
  /** (Internal) Gets a nice type name for the system, permitting overriding.
  */
  virtual std::string DoGetSystemType() const {
    return NiceTypeName::Get(typeid(*this));
  }

  friend std::string
  drake::internal::GetSystemType(const SystemBase& thing);
...
std::string GetSystemType(const systems::SystemBase& thing) {
  return thing.DoGetSystemType();
}
